...
contains
   ...
   !====================================================================
   !Function to compute the blackbody radiation 
   !intensity at a given spectral band
   !====================================================================
   real(dp) function Ib_function(tttmp,x_eta)
      
      !-----------------------------------------------------------------
      !Declaration of variables
      !-----------------------------------------------------------------
      use constants, only: c_planck1,c_planck2
      implicit none
      real(dp),intent(in) :: tttmp,x_eta

      !-----------------------------------------------------------------
      !Computing the function
      !-----------------------------------------------------------------
      if (tttmp.eq.0._dp) then
         !Assure the function to be zero for null temperatures
         Ib_function = 0._dp
         
      else
         !Eq. (1.19) [Howell et al., 2010]
         Ib_function = 2._dp*c_planck1*(x_eta**3._dp)/&
                        (DEXP(c_planck2*x_eta/tttmp)-1._dp+small)
      endif
   ...
   real(dp) function integrate_Ib(ttmp,eta_minus,eta_plus)
      ...
      int_ib_loop: do
         ...
         do while(curr_eta.le.eta_plus)
            ib_sum = ib_sum + Ib_function(ttmp,curr_eta)*deta           !Compute the Ib integration
