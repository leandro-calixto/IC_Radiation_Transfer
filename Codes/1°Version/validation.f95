...
!                       xs(id_h2o,:,:,:) = 0.2_dp
            ...
            do i=2,xpoints-1                               
               selectcase(validation_subcase)              
                  case(1)
                     ...
                     xs(id_co2,i,j,k) = 0.1_dp
                  case(4)                                  
                     T(i,j,k) = 1000._dp + 500._dp*dcos(pi*x(i) / 10._dp)                    
                     xs(id_h2o,i,j,k) = 0.5_dp - 0.5_dp*dcos(pi*x(i) / 10._dp)
                     xs(id_co2,i,j,k) = 0.5_dp + 0.5_dp*dcos(pi*x(i) / 10._dp)
               endselect
            enddo
         ...
         case('solovjovJQSRT2011')
            ...
            do i = 1, xpoints
               select case(validation_subcase)
                     case(2) 
                        T(i,j,k) = 1000._dp + 250._dp*dcos(pi*x(i)/(2._dp))  
                        xs(id_h2o,i,j,k) = 0.1_dp
                        
                     case(3) 
                        T(i,j,k) = 500._dp + 1000._dp*dsin(pi*x(i)/10._dp)  
                        xs(id_h2o,i,j,k) = dsin(pi*x(i)/10._dp)  
               end select
            enddo
